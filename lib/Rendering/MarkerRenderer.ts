  ...
  render(time, markers) {
    if (getSetting('showMarkersSong')) {
      ...
      markers.forEach((marker) => {
        if (marker.timestamp / 1000 >= time && marker.timestamp / 1000 < lookAheadTime) {
          let y = this.renderDimensions.getYForTime(marker.timestamp - time * 1000);
          let txtWd = c.measureText(marker.text).width;
          c.fillText(marker.text, this.renderDimensions.windowWidth / 2 - txtWd / 2, y + 3);
          c.beginPath();
          c.moveTo(0, y);
