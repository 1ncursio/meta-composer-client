...
import { roundToDecimals } from '@utils/util';
...
export class DebugRender {
  ...
  constructor(active, ctx: CanvasRenderingContext2D, renderDimensions: RenderDimensions) {
    ...
    if (getSetting('showNoteDebugInfo')) {
      ...
      Object.keys(renderInfos).forEach((trackIndex) => {
        renderInfos[trackIndex].black
          .filter((renderInfo) => this.isMouseInRenderInfo(renderInfo, mouseX, mouseY))
          .forEach((renderInfo) => {
  ...
  drawNoteInfoBox(renderInfo, mouseX, mouseY, amountOfNotesDrawn) {
    ...
    let top = mouseY > this.renderDimensions.windowHeight / 2 ? -10 - 14 * lines.length : 10;
