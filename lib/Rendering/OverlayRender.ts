...
export class OverlayRender {
   ...
   */
  addOverlay(message: string, duration: number) {
  ...
  render() {
    ...
    if (this.overlays.length) {
      this.globalAlpha = this.setAlphaForOverlay(this.overlays[this.overlays.length - 1]);
      this.ctx.fillStyle = 'rgba(0,0,0,0.9)';
      this.ctx.fillRect(0, 0, this.renderDimensions.windowWidth, this.renderDimensions.windowHeight);
    }
