...
import { getScene, getTexture, getTheeJsColor, loadTexture } from './threeJsHandler.js';
...
export class ThreeJsParticle {
  ...
  addTextureCallbacks() {
    ...
    setSettingCallback('particlesTexture', () => loadTexture(getSetting('particlesTexture')));
    ...
    let resetShaderOpts = ['particleLife', 'particleAmount'];
      ...
      setSettingCallback(setting, () => {
        ...
        if (cache) {
          for (let color in this.particleCache[currentType]) {
            this.particleCache[currentType][color].material.uniforms[setting].value = getSetting(setting);
          }
        } else {
    ...
    this.renderDimensions.registerResizeCallback(() => {
      ...
      for (let type in this.particleCache) {
        for (let color in this.particleCache[type]) {
          ...
          cache.material.uniforms.screenWidth.value = this.renderDimensions.windowWidth;
          cache.material.uniforms.screenHeight.value = this.renderDimensions.windowHeight;
        }
      }
    });
  }
  ...
  create(renderInfo) {
    ...
    optsList.forEach((opts) => {
      this.createParticlesOfType(renderInfo, opts);
    });
  }
  render() {
  ...
  createParticlesOfType(renderInfo, opts) {
    let color = opts.particleOverrideColor ? opts.particleColor : renderInfo.fillStyle;
    let amount = parseFloat(opts.particleAmount > 1 ? Math.floor(opts.particleAmount) : opts.particleAmount);
    ...
    if (!cache) {
      this.particleCache[opts.particleTypeName][color] = new ThreeJsBuffer(
        this.renderDimensions,
        opts,
        color,
        bufferAmount,
      );
      cache = this.particleCache[opts.particleTypeName][color];
      let scene = getScene(opts.particleDistributionZ);

      scene.add(this.particleCache[opts.particleTypeName][color].points);
    }
