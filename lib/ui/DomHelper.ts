...
export class DomHelper {
  ...
  static createTextInputDiv(label, value, onChange) {
    ...
    cont.appendChild(this.createTextInput(onChange, {}, { value: value }));
  ...
  static createElement(tag: keyof HTMLElementTagNameMap, styles, attributes) {
    tag = tag || 'div';
    attributes = attributes || {};
    styles = styles || {};
    const el = document.createElement<typeof tag>(tag);
  ...
  static createDynamicInputSelect(title, items, value, selectCallback, renameCallback, createCallback, deleteCallback) {
    let selectBoxOuter = DomHelper.createDivWithIdAndClass(title, 'dynSettingContOuter');
    ...
    let addButton = DomHelper.createGlyphiconTextButton('add' + title, 'plus-sign', 'New', createCallback);

    let label = DomHelper.createElementWithClass('dynSettingLabel', 'label', {}, { innerHTML: title });
      ...
      list.forEach((item) => {
        let el = DomHelper.createDynamicInputSelectOption(
          ...
          () => {
            ...
            selectBoxInner.querySelectorAll('.dynSettingOptionOuter').forEach((opt) => {
  ...
  static createDynamicInputSelectOption(txt, selectCallback, renameCallback, deleteCallback) {
    let optionOuter = DomHelper.createDivWithIdAndClass(txt, 'dynSettingOptionOuter');

    let txtElement = DomHelper.createDivWithClass('dynSettingOptionLabel');
    txtElement.innerHTML = txt;

    let delButton = DomHelper.createGlyphiconButton('dynInputOptionDel' + txt.trim(), 'remove', deleteCallback);
    let renameButton = DomHelper.createGlyphiconButton('dynInputOptionRename' + txt.trim(), 'pencil', () => {
  ...
  static createInputSelect(title, items, value, callback) {
    ...
    selectBox.updateList = (list, val) => {
      selectTag.querySelectorAll('option').forEach((option) => option.parentNode.removeChild(option));
      list.forEach((item) => {
  ...
  static createColorPickerGlyphiconText(glyph, text, startColor, onChange) {
    ...
    let getGlyphEl = () =>
      glyphBut.querySelector('#colorPickerGlyph' + glyph + replaceAllString(text, ' ', '_') + ' .glyphicon');
  ...
  static createColorPickerText(text, startColor, onChange) {
    ...
    let colorPicker = Pickr.create({
      ...
      swatches: Object.keys(CONST.TRACK_COLORS)
        .map((key) => [CONST.TRACK_COLORS[key].white, CONST.TRACK_COLORS[key].black])
        .flat(),
      components: {
        hue: true,
        preview: true,
    ...
    colorPicker.on('change', (color) => {
      ...
      let roundedArr = [Math.floor(1 * arr[0]) / 1, Math.floor(1 * arr[1]) / 1, Math.floor(1 * arr[2]) / 1, arr[3]];
      let str = 'rgba(' + roundedArr[0] + ',' + roundedArr[1] + ',' + roundedArr[2] + ',' + roundedArr[3] + ')';
