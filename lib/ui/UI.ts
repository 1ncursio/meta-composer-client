...
import { initThreeJs } from '../Rendering/ThreeJs/threeJsHandler';
...
export class UI {
  constructor(render, isMobile) {
    initThreeJs(render.renderDimensions);
    ...
    render.renderDimensions.registerResizeCallback(this.refreshMenuHeight.bind(this));
    this.renderDimensions = render.renderDimensions;
  ...
  setInnerMenuDivsTop() {
    let cssHeight =
      'calc(100% - ' +
      (this.getNavBar().clientHeight + PROGRESS_BAR_CANVAS_HEIGHT + this.getCloseInnerMenuDivButton().clientHeight) +
      'px)';
    ...
    document.querySelector('.innerMenuDivsContainer').style.marginTop = PROGRESS_BAR_CANVAS_HEIGHT + 'px';
  }
  ...
  createControlMenu() {
    ...
    let leftTop0 = DomHelper.createElementWithClass('topContainer hiddenIfSmall');
    let leftTop1 = DomHelper.createElementWithClass('topContainer hiddenIfSmall');

    let middleTop = DomHelper.createElementWithClass('topContainer');
    let rightTop0 = DomHelper.createElementWithClass('topContainer ');
    let rightTop1 = DomHelper.createElementWithClass('topContainer hiddenIfSmall');
    let rightTop2 = DomHelper.createElementWithClass('topContainer hiddenIfSmall');

    let sandwichMenuTop = DomHelper.createElementWithClass('topContainer shownIfSmall');
  ...
  getFileButtonGroup() {
    ...
    DomHelper.appendChildren(fileGrp, [this.getLoadSongButton(), this.getLoadedSongsButton()]);
    return fileGrp;
  }
  ...
  getSettingsButton() {
    if (!this.settingsButton) {
      ...
      this.settingsButton.onclick = this.getShowHideListener(this.getSettingsDiv(), this.settingsButton).bind(this);
    }
  ...
  getLoadSongButton() {
    if (!this.loadSongButton) {
      this.loadSongButton = DomHelper.createFileInput('Load MIDI', this.handleFileSelect.bind(this));
  ...
  getLoadedSongsButton() {
    if (!this.loadedSongsButton) {
      this.loadedSongsButton = DomHelper.createGlyphiconTextButton('songsBtn', 'music', 'Songs');
      this.loadedSongsButton.onclick = this.getShowHideListener(this.getLoadedSongsDiv(), this.loadedSongsButton);
    ...
    if (!this.tracksButton) {
      let onShowCallback = () => {
        ...
        document
          ...
          .forEach((el) => (el.innerHTML = getPlayer().getCurrentTrackInstrument(el.id.split('instrumentName')[1])));
      };
      this.tracksButton = DomHelper.createGlyphiconTextButton('tracks', 'align-justify', 'Tracks');
  ...
  getMidiSetupButton() {
    if (!this.midiSetupButton) {
      this.midiSetupButton = DomHelper.createGlyphiconTextButton('midiSetup', 'tower', 'Midi-Setup');
      this.midiSetupButton.onclick = this.getShowHideListener(this.getMidiSetupDialog(), this.midiSetupButton);
  ...
  getChannelsButton() {
    if (!this.channelsButton) {
      ...
      this.channelsButton = DomHelper.createGlyphiconTextButton('channels', 'align-justify', 'Channels');
      this.channelsButton.onclick = this.getShowHideListener(this.getChannelMenuDiv(), this.channelsButton);
  ...
  hideAllDialogs() {
    ...
    document.querySelectorAll('.centeredMenuDiv').forEach((el) => {
      DomHelper.hideDiv(el);
    });
  ...
  newSongCallback() {
    ...
    if (getCurrentSong().activeTracks.length > 2 && getSetting('enableSheet')) {
      Notification.create(
  ...
  getMidiSetupDialog() {
    if (!this.midiSetupDialog) {
      this.midiSetupDialog = DomHelper.createDivWithIdAndClass('midiSetupDialog', 'innerMenuDiv');
      ...
      let text = DomHelper.createDivWithClass('centeredBigText', { marginTop: '25px' }, { innerHTML: textMsg });
  ...
  createOutputDeviceDiv(device) {
    ...
    if (getMidiHandler().isOutputDeviceActive(device)) {
      document
        ...
        .forEach((el) => (el.classList.contains('selected') ? el.classList.remove('selected') : null));
      DomHelper.addClassToElement('selected', deviceDiv);
    }
  ...
  getSandwichMenuButton() {
    if (this.sandwichMenuBtn == null) {
      this.sandwichMenuBtn = DomHelper.createGlyphiconButton('sandwichMenu', 'list');
      this.sandwichMenuBtn.onclick = this.getShowHideListener(this.getSandwichMenu(), this.sandwichMenuBtn);
  ...
  getSandwichMenu() {
    if (this.sandwichMenu == null) {
      this.sandwichMenu = DomHelper.createDivWithClass('sandwichMenu  innerMenuDiv hidden');
      let swBtns = [
  ...
  getSandwichLoadSongButton() {
    if (!this.swLoadSongButton) {
      this.swLoadSongButton = DomHelper.createFileInput('Upload MIDI', this.handleFileSelect.bind(this));
  ...
  getSandwichLoadedSongsButton() {
    if (!this.swLoadedSongsButton) {
      this.swLoadedSongsButton = DomHelper.createGlyphiconTextButton('swsongsBtn', 'music', 'Songs');
      this.swLoadedSongsButton.onclick = this.getShowHideListener(this.getLoadedSongsDiv(), this.swLoadedSongsButton);
      DomHelper.addClassToElement('floatSpanLeft', this.swLoadedSongsButton);
      DomHelper.addClassToElement('innerMenuDivButton', this.swLoadedSongsButton);
    }
    return this.swLoadedSongsButton;
  }
  getSandwichMidiSetupButton() {
    if (!this.swMidiSetupButton) {
      this.swMidiSetupButton = DomHelper.createGlyphiconTextButton('swmidiSetup', 'tower', 'Midi-Setup');
      this.swMidiSetupButton.onclick = this.getShowHideListener(this.getMidiSetupDialog(), this.swMidiSetupButton);
  ...
  getSandwichTracksButton() {
    if (!this.swTracksButton) {
      this.swTracksButton = DomHelper.createGlyphiconTextButton('swtracks', 'align-justify', 'Tracks');
      this.swTracksButton.onclick = this.getShowHideListener(this.getTrackMenuDiv(), this.swTracksButton);
  ...
  getSandwichSettingsButton() {
    if (!this.swSettingsButton) {
      this.swSettingsButton = DomHelper.createGlyphiconTextButton('settingsButton', 'cog', 'Settings');
      this.swSettingsButton.onclick = this.getShowHideListener(this.getSettingsDiv(), this.swSettingsButton);
  ...
  getSandwichMuteButton() {
    if (!this.swMuteButton) {
      this.swMuteButton = DomHelper.createGlyphiconTextButton('mute', 'volume-up', 'Mute', (ev) => {
        if (getPlayer().muted) {
          getPlayer().muted = false;
          if (!isNaN(getPlayer().mutedAtVolume)) {
            if (getPlayer().mutedAtVolume == 0) {
