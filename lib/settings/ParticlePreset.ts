...
import { getSavedParticlePresets, saveCurrentPresetSettings } from './LocalStorageHandler.js';
import { getDefaultSettingValue, getSetting, setSettingCallback, setSettingList, setSettingUI } from './Settings';
...
class ParticlePresetHandler {
  constructor() {
    ...
    this.customPresets = getSavedParticlePresets().map((presetOpts) => new ParticlePreset(presetOpts));
    let presetName = getSetting('particlePresetName') || 'Default';
    let savedPreset = this.customPresets.find((preset) => preset.particlePresetName == presetName);
    if (savedPreset) {
    ...
    setSettingList('particlePreset', this.getAllPresetNames(), this.currentPreset.particlePresetName);

    this.dontSave = false;
    this.disableListeners = false;
  }
  getDefaultPresets() {
    return getDefaultPresets().map((presetOpts) => new ParticlePreset(presetOpts, true));
  }
  initListeners() {
    ...
    Object.keys(tmpType).forEach((attrName) => setSettingCallback(attrName, () => this.onSettingChanged(attrName)));

    setSettingCallback('particlePresetName', () => {
      if (this.disableListeners) return;

      if (this.currentPreset.isDefault && this.currentPreset.particlePresetName != getSetting('particlePresetName')) {
        this.createAndSetPreset();
      } else {
        this.currentPreset.particlePresetName = getSetting('particlePresetName');
      }
      setSettingList('particlePreset', this.getAllPresetNames(), getSetting('particlePresetName'));
    });
    setSettingCallback('particleTypeName', () => {
      if (this.disableListeners) return;
      this.currentType.particleTypeName = getSetting('particleTypeName');
      setSettingList('particlePresetType', this.getCurrentPresetTypeNames(), this.currentType.particleTypeName);
    });
  }
  createAndSetPreset() {
    this.disableListeners = true;
    ...
    setSettingList('particlePreset', this.getAllPresetNames(), newPreset.particlePresetName);
  ...
  createAndSetType() {
    ...
    setSettingList('particlePresetType', this.getCurrentPresetTypeNames(), newType.particleTypeName);
  ...
  setPreset(preset) {
    ...
    try {
      setSettingList('particlePresetType', this.getCurrentPresetTypeNames(), firstType.particleTypeName);
    } catch (e) {
      console.error(e);
    }
    this.disableListeners = false;
  ...
  onSettingChanged(settingId) {
    ...
    if (this.currentPreset.isDefault && this.currentType[settingId] != getSetting(settingId)) {
      this.createAndSetPreset();
    }
...
export const deleteParticlePreset = (presetName) => {
  let preset = thePresetHandler.getPresetByName(presetName);
  if (preset) {
    ConfirmDialog.create('Are you sure you want to delete the Preset ' + preset.particlePresetName + '?', () => {
      if (preset.isDefault) {
        Notification.create('Cannot delete a default preset.', 2000);
        return;
      }
      getPresetHandler().customPresets.splice(getPresetHandler().customPresets.indexOf(preset), 1);
...
export const deleteCurrentParticlePreset = () => {
  ConfirmDialog.create(
    'Are you sure you want to delete the Preset ' + getPresetHandler().currentPreset.particlePresetName + '?',
    () => {
      if (getPresetHandler().currentPreset.isDefault) {
        Notification.create('Cannot delete a default preset.', 2000);
        return;
      ...
      getPresetHandler().customPresets.splice(
        getPresetHandler().customPresets.indexOf(getPresetHandler().currentPreset),
        1,
      );
      getPresetHandler().setPreset(getPresetHandler().defaultPresets[0]);
      setSettingList(
...
export const deleteParticleType = (typeName) => {
  ...
  if (type) {
    ConfirmDialog.create('Are you sure you want to delete the particle type ' + type.particleTypeName + '?', () => {
      if (preset.isDefault) {
        Notification.create('Cannot delete a default preset type.', 2000);
        return;
      }
      if (thePresetHandler.currentPreset.types.length < 2) {
        Notification.create('Cannot delete. At least one type per preset required.', 2000);
        return;
      }

      getPresetHandler().currentPreset.types.splice(getPresetHandler().currentPreset.types.indexOf(type), 1);
...
export const deleteCurrentParticleType = () => {
  ConfirmDialog.create(
    'Are you sure you want to delete the Type ' + getPresetHandler().currentType.particleTypeName + '?',
    () => {
      ...
      getPresetHandler().currentPreset.types.splice(
        getPresetHandler().currentPreset.types.indexOf(getPresetHandler().currentType),
        1,
      );
      getPresetHandler().setType(getPresetHandler().currentPreset.types[0]);
      setSettingList(
