...
import { formatNote, getKeyManagerNoteName, getVexFlowNoteName, getWhiteKeyNumber } from '../Util.js';
...
export class SheetRender {
  ...
  svgNoteHead: HTMLImageElement;
  svgSharpAcc: HTMLImageElement;
  svgFlatAcc: HTMLImageElement;
  svgNaturalAcc: HTMLImageElement;
  lastMin: number;
  lastMax: number;
  ...
  render(curTime: number, sheetGen: SheetGenerator, inputNotes, mouseX: number, mouseY: number) {
    ...
    if (window.innerHeight / 3 < unscaledHeight) {
      this.scale = window.innerHeight / 3 / unscaledHeight;
    } else {
      this.scale = 1;
    }
    this.computeStaffYs(sheetGen);
    ctx.clearRect(-10, -1000, this.renderDimensions.windowWidth + 20, window.innerHeight + 1000);
    ctx.save();
    if (this.scale != 1) {
    ...
    let curPlayMeasureIndex = this.getCurrentlyPlayedMeasureIndex(sheetGen, curTsp);
    let curPlayMeasure = sheetGen.getMeasure(Math.max(0, curPlayMeasureIndex));
    let trackIndex = 0;

    let cursorX = this.getCursorXPosition(sheetGen, curPlayMeasure, trackIndex, curTsp, curPlayMeasureIndex);

    //offset for scrolling sheet
    let xOffset = this.getXOffset(cursorX + this.getMargin(sheetGen, curPlayMeasureIndex));

    if (getSetting('renderSheetCursor')) {
      this.renderCursorBar(cursorX - xOffset + this.getMargin(sheetGen, curPlayMeasureIndex));
    }

    let measuresToDraw = this.getMeasuresToDraw(sheetGen, curPlayMeasureIndex, cursorX);
  ...
  renderMeasures(sheetGen: SheetGenerator, curTsp: number, measuresToDraw, xOffset: number) {
    let ctx = this.ctx;
    let tracks = Object.keys(measuresToDraw[0].measure.staves);
    let enabledTracks = Object.keys(tracks).filter((track) => isTrackSheetEnabled(track));
    ...
    measuresToDraw.forEach((measureToDraw) => {
      ...
      enabledTracks.forEach((track) => {
        ...
        if (measureIndex == 0 || sheetGen.lineStartIndexes.indexOf(measureIndex) > -1) {
          ctx.drawImage(
            keyCnv,
            0,
            0,
  ...
  renderActiveNotes(enabledTracks, ctx, x, measure, curTsp) {
    ...
    let activeNoteColor = isColorActiveNotes ? getSetting('sheetActiveNotesColor') : '';
    let highlightNoteColor = isHighlightActiveNotes ? getSetting('sheetActiveHighlightColor') : '';
  ...
  getMeasuresToDraw(sheetGen, curPlayMeasureIndex, cursorX) {
    let { firstDrawnMeasure, firstDrawnMeasureIndex } = this.getFirstDrawnMeasure(
      sheetGen,
      curPlayMeasureIndex,
      cursorX,
    );
    let nextLineFirstIndex = sheetGen.lineStartIndexes.find((index) => index > firstDrawnMeasureIndex);
    ...
    while (
      ...
      (isScrollingSheet && measureX - xOffset < window.innerWidth / this.scale && firstDrawnMeasure)
    ) {
      measuresToDraw.push({
    ...
    if (getSetting('sheetMeasureScroll')) {
      ...
      if (curPlayMeasureIndex > 0) {
        while (
          ...
          firstDrawnMeasure.cumulativeXPosition + firstDrawnMeasure.measureWidth - xOffset > 0
        ) {
          firstDrawnMeasureIndex--;
          firstDrawnMeasure = sheetGen.getMeasure(firstDrawnMeasureIndex);
        }
  ...
  getCursorXPosition(sheetGen, curPlayMeasure, trackIndex, curTsp, curPlayMeasureIndex) {
    let hasNotes = this.isMeasureHasNotes(curPlayMeasure, trackIndex);
    let beforeFirstInMeasure =
      hasNotes && this.getFirstNoteOfMeasure(trackIndex, curPlayMeasure, curPlayMeasureIndex).note.timestamp >= curTsp;
    let afterLastInMeasure =
      hasNotes && this.getLastNoteOfMeasure(trackIndex, curPlayMeasure, curPlayMeasureIndex).note.timestamp < curTsp;
    ...
    if (beforeFirstInMeasure) {
      ...
      nextNote = hasNotes
        ? this.findNextUnplayedInMeasure(trackIndex, curPlayMeasure, curPlayMeasureIndex, curTsp)
        : null;
      nextTsp = nextNote ? nextNote.note.timestamp : curPlayMeasure.endTsp;
    } else if (afterLastInMeasure) {
      lastNote = hasNotes
        ? this.getLatestPlayedInMeasure(trackIndex, curPlayMeasure, curPlayMeasureIndex, curTsp)
        : null;
      lastTsp = lastNote ? lastNote.note.timestamp : curPlayMeasure.startTsp;
      nextNote =
      ...
      lastNote = this.getLatestPlayedInMeasure(trackIndex, curPlayMeasure, curPlayMeasureIndex, curTsp);
      lastTsp = lastNote.note.timestamp;
      nextNote = this.findNextUnplayedInMeasure(trackIndex, curPlayMeasure, curPlayMeasureIndex, curTsp);
      nextTsp = nextNote.note.timestamp;
    }

    let leftX = 0;
    if (lastIsInPreviousMeasure) {
      leftX = sheetGen.getMeasure(curPlayMeasureIndex - 1).cumulativeXPosition + lastNote.staveNote.getNoteHeadBeginX();
    } else {
      leftX = lastNote
        ? curPlayMeasure.cumulativeXPosition + lastNote.staveNote.getNoteHeadBeginX()
    ...
    if (nextIsInNextMeasure) {
      rightX =
        sheetGen.getMeasure(curPlayMeasureIndex + 1).cumulativeXPosition + nextNote.staveNote.getNoteHeadBeginX();
    } else {
      rightX = nextNote
        ? curPlayMeasure.cumulativeXPosition + nextNote.staveNote.getNoteHeadBeginX()
        : curPlayMeasure.cumulativeXPosition + curPlayMeasure.measureWidth;
    }

    let cursorX = leftX + ((curTsp - lastTsp) / (nextTsp - lastTsp)) * (rightX - leftX);

    return isFinite(cursorX) ? cursorX : 0;
  }
  getXOffset(cursorX) {
    if (getSetting('sheetMeasureScroll')) {
      if (cursorX > this.renderDimensions.windowWidth / 3) {
        return cursorX - this.renderDimensions.windowWidth / 3;
      }
    }
    return 0;
  }
  ...
  getSheetLineWidth(sheetGen, measureIndex) {
    ...
    let firstLineIndex = lineIndexes[Math.max(0, lineIndexes.indexOf(nextLineIndex) - 1)];
  ...
  getFirstTrackWithNotes(curPlayMeasure) {
    ...
    for (let track in curPlayMeasure.roundedNotes) {
      for (let tsp in curPlayMeasure.roundedNotes[track]) {
        if (curPlayMeasure.roundedNotes[track][tsp].find((roundedNote) => !roundedNote.isRest)) {
          return track;
        }
      }
    }
  ...
  isMeasureHasNotes(measure, trackIndex) {
    ...
    return grouped.find((roundedNotes) => roundedNotes.find((roundedNote) => !roundedNote.isRest));
  }
  ...
  drawMouseHoverNotes(x, y, measureCnv, track, measure) {
    if (
      ...
      c.fillRect(
        x + hoveredNotes[0].staveNote.getNoteHeadBeginX() + hoveredNotes[0].staveNote.width / 2 - txtWd / 2 - 4,
        y - 10,
        txtWd + 8,
        textY + 10,
      );
      ...
      c.fillText(
        txt,
        x + hoveredNotes[0].staveNote.getNoteHeadBeginX() + hoveredNotes[0].staveNote.width / 2 - txtWd / 2,
        y + textY - 6,
      );
    }
    return measure;
    ...
    for (let groupedNoteKey in notesInTrack) {
      ...
      let dis = Math.abs(
        this.mouseX / this.scale -
          x -
          (groupedNote[0].staveNote.getNoteHeadBeginX() + groupedNote[0].staveNote.width / 2),
      );
  ...
  getActiveNotes(measure, curTsp: number, track) {
    let activeNotes = [];
    ...
    if (measure.startTsp < curTsp && measure.endTsp > curTsp) {
      Object.values(measure.roundedNotes[track]).forEach((groupedRoundedNotes) =>
        groupedRoundedNotes.forEach((roundedNote) => {
          if (roundedNote.note.timestamp < curTsp && roundedNote.note.timestamp + roundedNote.note.duration > curTsp) {
  ...
  renderInputNotes(sheetGen, cursorX, ctx, inputNotes, measure, curTime) {
    if (measure.canvases && measure.canvases[0]) {
      inputNotes.forEach((inputNote) => {
        this.renderInputNote(ctx, sheetGen, measure, inputNote, curTime, cursorX);
      });
    }
  }
  renderInputNote(ctx, sheetGen, measure, inputNote, curTime, cursorX) {
    ...
    Object.entries(measure.staves).forEach((staffEntry) => {
      ...
      let noteDif = getWhiteKeyNumber(inputNote.noteNumber + 21) - getWhiteKeyNumber(midNoteNum);
    ...
    Object.entries(notesOfStaff)
      ...
      .forEach((entry) =>
        entry[1].forEach((roundedNote) =>
          keyManager.selectNote(getKeyManagerNoteName(roundedNote.note.midiNoteNumber)),
        ),
      );

    let noteParts = keyManager.selectNote(getKeyManagerNoteName(inputNote.noteNumber + 21));
    let noteAccidental = noteParts.accidental === null && noteParts.change ? 'n' : noteParts.accidental;
    let line = sheetGen.getLineForKey(noteParts.note + '/' + noteName[noteName.length - 1], clef);

    let y = bestStaffY.min + (bestStaff.space_above_staff_ln + 4.5 - line) * bestStaff.spacing_between_lines_px;
  ...
  computeStaffYs(sheetGen) {
    ...
    if (this.ctx.canvas.height != Math.floor(curY * this.scale)) {
      ...
      this.ctx.canvas.style.bottom = -this.ctx.canvas.height + 2 - PROGRESS_BAR_CANVAS_HEIGHT + 'px';
      this.renderDimensions.setSheetHeight(curY);
      this.canvasTop = this.ctx.canvas.getBoundingClientRect().top;
      sheetGen.computeLines(this.renderDimensions.windowWidth / this.scale, SHEET_SIDE_MARGIN / this.scale);
    ...
    return Math.max(
      ...
      sheetGen.measures.indexOf(
        sheetGen.measures.filter(
          (measure, index) =>
            measure[0] <= curTsp &&
            (sheetGen.measures.length - 1 < index + 1 || sheetGen.measures[index + 1][0] > curTsp),
        )[0],
      ) + 1,
    );
  }
  ...
  findNextUnplayedInMeasure(trackIndex, measure, measureIndex, curTsp) {
    let notesOfTrack = this.getFlattenedNotesOfTrack(trackIndex, measure, measureIndex);
