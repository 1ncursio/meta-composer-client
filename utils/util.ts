 ...
 */
function getWhiteKeyNumber(noteNumber: number) {
 ...
 * @param {Number} noteNumber
 */
function isBlack(noteNumber: number) {
...
function loadJson(url: string, callback) {
...
function nFormatter(num: number, digits) {
  var si = [
      {
        value: 1e100,
        symbol: "It's Enough",
...
function getCssVariable(aVarName: string) {
  return getComputedStyle(document.documentElement).getPropertyValue('--' + aVarName);
}

function setCssVariable(aVarName: string, value: string) {
...
function formatNote(midiNoteNumber: number) {
...
function getNoteName(midiNoteNumber: number) {
  return CONST.MIDI_NOTE_TO_KEY[midiNoteNumber];
}

function getVexFlowNoteName(midiNoteNum: number) {
...
function getKeyManagerNoteName(midiNoteNum: number) {
  let name = getNoteName(midiNoteNum);
  let spl = name.split('b');
  if (spl.length > 1) {
    return spl[0] + 'b';
