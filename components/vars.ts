export const normalYPosition = 1.5;
export const hiddenYPosition = 1000;
...
export const getUniqueId = (stringPrefix: string) => {
  var datestr = new Date().getTime().toString();
  var randomstr = Math.random().toString().replace('.', '');
  return stringPrefix + '_' + datestr + randomstr;
};

// TODO: 사용되지 않는 것 같아서 주석 처리.
// const getTextWidth = (text: string, font: string) => {
//   // re-use canvas object for better performance
//   //   const canvas = getTextWidth.canvas || (getTextWidth.canvas = document.createElement<'canvas'>('canvas'));
//   let canvas;
//   if (!getTextWidth.canvas) {
//     getTextWidth.canvas = document.createElement<'canvas'>('canvas');
//   }
...
export const drawText = (
  ...
  if (textString.match('char#')) {
    const char = parseInt(textString.substring(textString.indexOf('#') + 1));
    if (align == 'left') {
...
type IconFont = keyof typeof icon_font;
