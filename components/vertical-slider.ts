...
export default AFRAME.registerComponent('gui-vertical-slider', {
  schema: {
  ...
  init: function () {
    ...
    el.setAttribute(
      'material',
      `shader: flat; opacity: ${data.opacity};  alphaTest: 0.5; color: ${data.backgroundColor}; side:front;`,
    );
    ...
    sliderBar.setAttribute(
      'geometry',
      `primitive: box; height: ${sliderHeight - data.percent * sliderHeight}; width: ${data.sliderBarWidth}; depth: ${
        data.sliderBarDepth
      };`,
    );
    sliderBar.setAttribute(
    ...
    hoverIndicator.setAttribute(
      'geometry',
      `primitive: box; height: 0.02; width: ${guiItem.width * 0.5}; depth: ${data.sliderBarDepth};`,
    );
  ...
  tick: function () {
    ...
    if (!intersection) {
      ...
      var localCoordinates = new Vector3();
